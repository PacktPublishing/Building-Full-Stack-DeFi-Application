...
import { toString, getLiquidityPools } from '../../utils/Helper';

const CreateFarmingPool = () => {
  ...
  useEffect(() => {
    if (active) {
      getLiquidityPools().then(pools => setLiquidityPools(pools));
      library.getBlockNumber().then(number => setCurrentBlock(number));
    }
  }, [active, library]);
  ...
  return (active ? <Grid container sx={{ width: { xs: "90vw", md: "40vw" } }}>
    <Grid item>
      <Grid container columnGap={12}>
        ...
        <Grid item>
          <Typography sx={{ mt: 1 }}>Create Farming Pool</Typography>
        </Grid>
      </Grid>
      <Divider sx={theme.component.divider} />
      <Grid container spacing={2}>
        <Grid item xs={6}>
          <Typography sx={{ mt: 2 }} >Staked Liquidity</Typography>
        </Grid>
        <Grid item xs={6}>
          <Button
            sx={theme.component.selectButton}
            endIcon={<KeyboardArrowDownIcon />}
            onClick={() => { setOpenModalStaked(true); setTokenIndex(0); }}
          >
            {Object.keys(stakedToken).length === 0 ? "Select a liquidity pool" : stakedToken.symbol}
          </Button>
        </Grid>
        <Grid item xs={6}>
        ...
        <Grid item xs={6}>
          <Button
            sx={theme.component.selectButton}
            endIcon={<KeyboardArrowDownIcon />}
            onClick={() => { setOpenModalReward(true); setTokenIndex(1); }}
          >
      ...
      <TokenSelectModal open={openModalReward}
        handleClose={() => setOpenModalReward(false)}
        selectToken={handleSelectToken}
        erc20Only={true}
      />
      <TokenSelectModal open={openModalStaked}
        handleClose={() => setOpenModalStaked(false)}
        selectToken={handleSelectToken}
        customTokens={Array.from(liquidityPools.entries()).map((value, i) => {
          return {
            "address": value[0],
  ...
  </Grid> : <Typography>Please connect to a wallet to create farming pool</Typography>);
}
