...
const ListFarmingPools = () => {
  ...
  const handleHarvest = async (address) => {
    setLoading(true);
    try {
      const farmingPool = new ethers.Contract(address, StakingPoolABI, library.getSigner());
  ...
  }, [active, library, getFarmingPools]);
  ...
  return <>
    <Grid container direction="column">
      {active ? <>
        ...
        {farmingPools.length > 0 ? farmingPools.filter(p => hideExpired ? p.rewardEndBlock > currentBlock : true).map((item, index) =>
          <Accordion
            key={`farming-pool-${index}`}
          ...
          </Accordion>) : <Typography>No Farming Pool Found</Typography>}
      </> : <Typography>Please connect to a wallet to view farming pools.</Typography>}
    </Grid>
    {active && <Grid container sx={{ mt: 2 }}>
      <Grid item xs={12}>
        <Button sx={theme.component.primaryButton} fullWidth onClick={
          () => navigate("create")
        }>Create Farming Pool</Button>
      </Grid>
    </Grid>}
  </>;
}
